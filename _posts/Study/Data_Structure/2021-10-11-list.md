...
```
배열은 인덱스를 가지고 있으며, 
순차적으로 데이터가 삽입 삭제될 수 있는 형태의 자료구조.
...
## 연결 자료구조로 구현한 리스트
### Linear Linked List
<img src="https://static.studytonight.com/data-structures/images/node-in-linked-list.png">

```
연결리스트는 자료들을 임의의 기억공간에 기억시키되 
자료 항목의 순서에 따라 노드의 포인터 부분을 이용하여 서로 연결시킨 자료구조.

연결을 위한 포인터 부분이 필요하기 때문에 
순차 리스트, 배열에 비해 기억공간 이용 효율이 좋지는 않음.

또한 연결 리스트 중 중간 노드의 연결이 끊어지면 그 다음 노드를 찾기 힘들지만, 
노드의 연결이 끊어지지만 않는다면 중간에 삽입 삭제가 용이함.
```
```
...
```
원형 연결 리스트는 단순 연결 리스트에서 마지막 노드가
리스트의 첫 번째 원소를 가리키게 하여 리스트 구조를 원형으로 만든 것임.
```

<img src="https://blog.kakaocdn.net/dn/bMvKmi/btqIrurapQg/A7z57ekv8TD9xdU1zO9XkK/img.png">
...
<img src="https://blog.kakaocdn.net/dn/O4idd/btqOxsof0NR/wZxCgtR4lmWqmWagaSWGh0/img.png">
