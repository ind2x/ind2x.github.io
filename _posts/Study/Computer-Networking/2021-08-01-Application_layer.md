...
운영체제 용어에서 실제로 통신하는 것은 프로그램이 아니라 **프로세스임**. 프로세스는 **```end system```** 에서 실행되는 **프로그램**임.
...
송신 호스트는 **메시지가 전달되어야 하는 수신 호스트의 주소와 수신 호스트에서 수행되고 있는 수신 프로세스도 식별**해야 함. 따라서 목적지 포트 번호가 수신 프로세스 식별을 위해 사용됨.
