...
+ **네트워크 계층**은 하나 이상의 트랜스포트 계층 프로토콜이 사용 가능하며 **대표적으로 인터넷은 TCP, UDP라는 두 가지 트랜스포트 계층 프로토콜을 가지고 있음.**
...
+ 트랜스포트 계층 프로토콜들은 ```end system```안에서 애플리케이션 프로세스에서 **네트워크 경계(네트워크 계층)까지** 메시지를 운반   
  (또한 반대 방향으로도 운반, 네트워크 계층 -> 앱 프로세스)
...
즉, IP는 통신하는 호스트 간 세그먼트 전달에는 최선을 다하지만 손실이나 순서 등 **어떠한 보장도 하지 않는다는 것**. 또한 무결성도 제공하지 않아 **비신뢰적인 서비스**라고도 함.
