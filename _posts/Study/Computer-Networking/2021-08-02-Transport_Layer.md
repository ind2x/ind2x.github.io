...
수신자는 만약 순서가 바뀐 패킷이 수신되면, 이미 전에 수신한 패킷에 대한 ACK 패킷을 전송한다. 

손상된 패킷이 수신되면, 수신자는 NAK 패킷을 전송한다. 
...
rdt 2.2는 비트 오류 패킷이 발생하면 NAK를 보내는 것이 아닌, 가장 최근에 정확하게 수신된 패킷 번호에 대한 ACK 보낸다.
