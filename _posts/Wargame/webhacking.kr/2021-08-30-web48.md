...
파일을 올리면 upload file이 있는데 경로를 확인해보니
/upload/test.txt이며 들어가면 파일 내용이 나옴.
...
rm -rf 파일명 이 서버에서 실행될 것임. 따라서 파일명;ls를 실행시켜주면 되기 때문에
파일명에 ;ls를 붙여서 올린 뒤 삭제하면 아래와 같이 나오게 됨.
```
```
