...
rank.php에서 보여지는 쿼리를 예상해보면 
select id, score from chall55 where score={$_GET['score']}
```
```
...
자세히 : https://dev.mysql.com/doc/refman/5.6/en/procedure-analyse.html
```
대략 이런식으로 나오는 듯함.
```
...
procedure analyse()를 이용한 sql injection 기법 : https://www.hides.kr/284
mysql XML Functions : https://dev.mysql.com/doc/refman/5.6/en/xml-functions.html
```
```
rank.php에서 score 값에 1 procedure analyse()를 치면 
webhacking.chall55.id 라는 DB명.테이블명.컬럼명을 볼 수 있음.
...
score에 필터링 되어있는게 있음. select, substr, mid, ', 등
