...
## Level 4 -> Level 5
```
ssh narnia@narnia.labs.overthewire.org -p 2226
```
```c
#include <string.h>
#include <stdlib.h>
...
```
입력을 ```[ebp-260]```부터 받음.  
따라서 ```119 nop + 25 shell + 120 nop + ret```
```
...
i에 500을 넣어줘야하기 때문에 i의 주소를 넣고 입력 값이 참조되어지는   
포맷 스트링은 첫 번재이므로 첫 번째에 500을 넣어줘야 함.  
