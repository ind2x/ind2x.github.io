...
## Solution
```
플래그 흭득 조건: if문을 통과를 해야 flag.txt 파일을 include 시켜서 플래그를 흭득
```
parse_url()함수란?  
Link: <a href="http://b.redinfo.co.kr/65" target="_blank">parse_url()</a>
```
요약하면 parse_url()는 URL 형태의 문자열을 받아와서 
scheme,host, port, user, pass, path, query, fragment 의 값을 반환하는데 
URL 문자열에 있는 값만 반환함. 
...
```
```
문제에서는 query값에 flag가 없으면 되므로 slash를 3개를 쓰면은 
if문을 통과하므로 flag.txt 파일을 볼 수 있다.
```
