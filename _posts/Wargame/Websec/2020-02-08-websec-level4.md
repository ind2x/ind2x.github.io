...
## Solution
```이 문제는 unserialize 함수를 이용한 **PHP object injection** 문제.```   
```unserialize함수는 serialize된 문자열을 PHP 값으로 반환해주는 함수다.```   
  
Link : <a href="https://blog.do9.kr/150" target="_blank">[PHP Object Injection]</a>   
Link : <a href="http://chongmoa.com/php/6902" target="_blank">[PHP Serialize/Unserialize]</a>  

```
코드를 보면 base64인코딩 된 쿠키 값을 디코딩 한 후 
unserialize해서 변수에 담는다. 
그 다음 주목할 부분은 try catch 구문이다.

sess_data 변수 값이 배열이 아니라면 
catch 구문으로 이동을 하고 예외상황을 출력한다. 
...
``` 
```
이 문자열을 base64인코딩 해줘서 쿠키값으로 주면 catch 구문에서 sess_data 값을 출력을 해주므로 
SQL 객체를 실행시켜서 소멸자에서 query를 실행시켜서 username에 플래그 값이 나온다. 
```
