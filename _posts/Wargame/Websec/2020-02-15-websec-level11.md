...
```
```
(select 2 id,enemy username from costume where id like 1)을 table 값으로, 
id값으로 2를 주면 플래그가 나옴.  
```
```sql
...
이 쿼리문으로 보면 id 값에는 2가 들어가게 되고 
enemy 값이 username 컬럼명으로 바뀌어서 출력이 되므로 
username 부분에서 플래그가 나옴. 
```
