...
sqlite에서  MySQL의 information_schema와 같은 역할을 하는 테이블은 sqlite_master.

컬럼으로는 몇개가 있었는데 필요한 컬럼은 sql컬럼(mysql의 information_schema 역할).
...
```
이 부분에서 터진다. 
union을 이용한 기초적인 sql injection 문제다. 

입력 : 1 union select 1,sql from sqlite_master -- 
```
```
