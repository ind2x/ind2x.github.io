...
유휴<sub>idle</sub> 스캐닝은 출발지 주소를 유휴 호스트의 주소로 스푸핑한 패킷을 대상 호스트에 보낸 후 유휴 호스트의 상태 변화를 관찰하는 스캔 방법이다.


<br>

아래 링크는 Nmap IDLE Scan의 탐지 방법에 대한 논문이다.

Link : <a href="https://scienceon.kisti.re.kr/srch/selectPORSrchArticle.do?cn=DIKO0015066837&dbt=DIKO" target="_blank">스캔 패킷 특징을 이용한 nmap idle 스캔의 효과적 탐지 방법</a>
