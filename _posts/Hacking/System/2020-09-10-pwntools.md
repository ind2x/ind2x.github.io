...
```python
p.send(value).encode('utf-8')  # value 송신

p.sendline(value).encode('utf-8') # value + '\n' 송신

p.sendafter('test', value).encode('utf-8') # test가 출력되면 value 송신

p.sendlineafter('test', value).encode('utf-8') # test가 출력되면 value + '\n' 송신
```
