...
```
```
디폴트는 max heap이므로 min heap으로 사용 시
2가지 방법이 있음.
```
```cpp
...
// 2. 값을 넣을 때 음수로 바꿔서 넣은 뒤 -pq.top()으로 출력.
...
각 요소가 데이터 값과 정렬 키를 갖고 있는 쌍인 컬렉션에서
데이터의 저장과 검색에 사용됨.
...
키 값은 중복되면 안됨. --> 중복되면 마지막에 넣은 값으로 갱신.
```
```cpp
template <class Key,
    class Type,
...
```
```
lower_bound, upper_bound를 이용해서 
특정 범위 내 원소의 개수, 특정한 값의 개수를 구할 수 있음.
```
```cpp
int main() {
