...
마지막 계단은 무조건 밟아야 하므로 마지막 계단 전 계단을 밟았을 때와 2칸 전의 계단을 밟았을 때로 구분하여 시도하였음.  
```dp[n] = n번째 계단을 밟았을 때 최대값```이라고 하면 ```dp[n] = max( f(n-1,1) , f(n-2,2) ) + s[n]```로 점화식을 세웠음.  
step은 0, 1, 2로 구분하였고 0은 ```문제에서 요구하는 값```이고 1은 ```n번째 계단에서 한 칸 뺀 n-1번째 계단```, 2는 ```n번째 계단에서 두 칸 뺀 n-2번째 계단```으로 하였음.  
...
이 코드의 문제점은 바로 n의 값임. n 값이 너무 크면 재귀함수의 속도가 너무 오래 걸리므로 시간초과가 발생하게 됨.. 
