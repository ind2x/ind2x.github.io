...
```
```
inline for문이나 if문 등 압축해서 표현하는 것은
기본적으로 짠 구조를 거꾸로 적어놓은 것 같음.
```
```python
# example
for i in check_nums :
...
**또한 C언어처럼 출력타입을 통해 출력할 수 있음.**  
**차이점은 변수 앞에 %를 붙여준다는 점.**  
```
>>> x = 12.3456789
...
함수로는 **input(), sys.stdin.readline()**이 있음.
```python
str = input()
str = sys.stdin.readline()

readline()이 input()보다 빠르다고 함.
```
...
여기서 readline(), readlines()는 개행문자인 "\n"도 같이 읽어옴.
```

## Python Built-in Functions
Link : <a href="https://docs.python.org/ko/3/library/functions.html" target="_blank">Built-in Functions</a>

## List
```
list[start:end] : 리스트의 start인덱스부터 end-1인덱스까지의 요소를 선택함.
슬라이싱에 값을 대입하는 것도 가능함. ex) letters[2:5]=[‘C’,‘D’,‘E’]
슬라이싱을 하면 요구된 요소를 포함하는 부분 리스트를 반환함. 즉, 새로운 복사본을 얻음
```
...
여러 개의 요소를 추가 가능. ex) .extend([9, 11, 13])
```
```
{list}.insert(인덱스 위치, 값)
...
key 매개변수를 이용하면 
요소들을 비교하기 전에 정렬 기준이 되는 함수를 적용시킬 수 있음.
```
```
ex) 
...
이때 문자열을 분리하는 분리자를 지정할 수 있으며, 
지정되지 않으면 스페이스를 이용하여 문자열을 분리함.
```

### 리스트 복사
```
score라는 리스트가 있을 때, value라는 변수를 value=score라 하면
value는 단지 score 리스트가 가리키고 있는 리스트를 가리키는 
score과 이름만 다른 변수가 됨.
...
## Tuple
```
튜플은 tuple=()로 생성하며, 리스트와 거의 비슷하나 
리스트와 달리 요소 값을 변경할 수 없음. 
...
### 요소 삭제
```
tuple은 말했듯이 immutable data로 list처럼 특정 요소를 삭제하는 것이 불가능.

따라서 tuple은 아예 변수를 삭제하는 것만 가능함. 이 때 del 를 사용하면 됨.
```

### tuple methods
튜플은 두 가지의 메소드만 사용 가능함.
```python
my_tuple = ('a', 'p', 'p', 'l', 'e',)
...
```
```
To make an empty set, use set() function.
```
...
두 함수의 차이점은 discard는 제거할 item이 set에 없어도 오류가 안나지만
remove 함수는 오류를 raise함.
...
### set operations
```
1. 합집합(union) : set1.union(set2), set1 | set2
2. 교집합(intersection) : set1.intersection(set2), set1 & set2
3. 차집합(difference) : set1(or 2).difference(set2(or 1)), set1(or 2) - set2(or 1)
```

## Function
keyword arguments는 반드시 positional arguments 보다 앞에 있어야 함.
``` python
# 2 keyword arguments
...
**3. try .. except 발생오류 as 오류 메시지 변수**
```
try:
    ...
except 발생 오류 as 오류 메시지 변수:
...
## lambda
anonymous function으로 이름이 없는 함수임.  
보통 함수는 def를 통해 이름을 갖게 됨.
```
...
```
```
두 개의 차이점은 global은 어느 함수에서나 사용 가능하지만 
nonlocal은 사용된 함수 바로 한단계 바깥쪽에 위치한 변수와 바인딩을 할 수 있음.
즉, 함수 한개를 정의하고 전역변수에 영향을 주게 하는 것은 안됨.
```
...
## class and object
```
__init__ : 생성자, 객체(class instance)가 생성되면 가장 먼저 실행되는 메소드

__class__ : __class__ 메소드를 통해 class 변수에 접근 가능.
...
class에서 메소드나 변수를 private으로 설정하고자 하면 
변수, 메소드 명 앞에 _ 또는 __를 붙임.
```
